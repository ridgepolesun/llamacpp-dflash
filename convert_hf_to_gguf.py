...
class DFlashDraftModel(Qwen3Model):
    """
    DFlash draft model (Qwen3-based block diffusion model for speculative decoding).
    Architecture: z-lab/Qwen3.5-4B-DFlash
    Differences from standard Qwen3:
    - No embed_tokens or lm_head (borrows from target model at inference)
    - Extra fc layer: [num_target_layers * hidden_size → hidden_size]
    - Extra hidden_norm: RMSNorm applied to fc output
    - 5 decoder layers with cross-attention to target hidden states
    """
    model_arch = gguf.MODEL_ARCH.QWEN3DFLASH

    def __init__(self, *args, **kwargs):
        # Call grandparent (TextModel) init directly to avoid Qwen3's reranker logic
        super(Qwen3Model, self).__init__(*args, **kwargs)

    def set_vocab(self):
        # DFlash draft model shares vocab with the target Qwen3 model.
        # The tokenizer config is not in the draft model directory; use gpt2-style fallback.
        try:
            self._set_vocab_sentencepiece()
        except FileNotFoundError:
            try:
                ...
                pass  # Vocab may not be needed for draft-only GGUF

    def set_gguf_parameters(self):
        # Write standard Qwen3 hyperparameters (skip Qwen3Model's reranker checks)
        super(Qwen3Model, self).set_gguf_parameters()

        # DFlash-specific parameters
        dflash_cfg = self.hparams.get("dflash_config", {})
        block_size = self.hparams.get("block_size", 16)
        num_target_layers = self.hparams.get("num_target_layers", 32)
        ...
        # Store the number of *selected* target layers (len of target_layer_ids),
        # since that is what determines the fc weight shape: [n_selected * n_embd, n_embd].
        ...
        # DFlash uses non-causal (bidirectional) attention within the draft block
        self.gguf_writer.add_causal_attention(False)

    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        # DFlash tensor names differ from standard HF Qwen3:
        # - "fc.weight"          → dflash.fc (global)
        # - "hidden_norm.weight" → dflash.hidden_norm (global)
        # - "norm.weight"        → output_norm.weight
        # - "layers.N.*"         → blk.N.* (standard per-layer tensors)
        # No embed_tokens or lm_head in the draft model.

        # Skip any unmapped tensors silently
        new_name = self.map_tensor_name(name, try_suffixes=(".weight", ".bias"))
        if new_name is None:
            return
