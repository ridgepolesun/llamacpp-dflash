...
class GGUFWriter:
    ...
    def add_dflash_block_size(self, block_size: int) -> None:
        self.add_uint32(Keys.LLM.DFLASH_BLOCK_SIZE.format(arch=self.arch), block_size)

    def add_dflash_num_target_layers(self, count: int) -> None:
        self.add_uint32(Keys.LLM.DFLASH_NUM_TARGET_LAYERS.format(arch=self.arch), count)

    def add_dflash_target_layer_ids(self, ids: list[int]) -> None:
