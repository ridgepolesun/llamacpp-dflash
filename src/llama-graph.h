...
public:
    ...
    // Return the hidden state output of layer `il` (set by graph builders that support it).
    // Returns nullptr if not captured.
    ggml_tensor * get_layer_hidden(int32_t il) const {
        if (il < 0 || il >= (int32_t)t_layer_hidden.size()) { return nullptr; }
        return t_layer_hidden[il];
    }
    ...
    // Per-layer hidden states (populated by graph builders that support DFlash).
    // Indexed by layer number; resize to n_layer before setting entries.
    std::vector<ggml_tensor *> t_layer_hidden;
