...
llm_build_qwen35::llm_build_qwen35(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Store layer output for DFlash hidden state extraction.
        if ((int32_t)res->t_layer_hidden.size() > il) {
            res->t_layer_hidden[il] = cur;
            ggml_set_output(cur);  // prevent ggml-alloc from reusing this buffer
            ggml_build_forward_expand(gf, cur);
        }
