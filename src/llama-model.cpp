...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_QWEN3DFLASH:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                ml.get_key(LLM_KV_DFLASH_BLOCK_SIZE,         hparams.n_dflash_block_size,        false);
                ml.get_key(LLM_KV_DFLASH_NUM_TARGET_LAYERS,  hparams.n_dflash_num_target_layers,  false);
                // Load target layer ids array
                {
                    std::vector<int32_t> ids;
                    if (ml.get_arr(LLM_KV_DFLASH_TARGET_LAYER_IDS, ids, false)) {
                        const int n_ids = (int)std::min(ids.size(), (size_t)8);
                        for (int i = 0; i < n_ids; i++) {
                            hparams.dflash_target_layer_ids[i] = ids[i];
                        }
                    }
                }
                type = LLM_TYPE_UNKNOWN; // small draft model, type not critical
            } break;
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_QWEN3DFLASH:
                {
                    // DFlash draft model: no tok_embd or output/lm_head
                    // These are borrowed from the target model at inference time.
                    output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);

                    // DFlash-specific global tensors
                    // fc: [n_embd, n_dflash_num_target_layers * n_embd]
                    const uint32_t n_target_layers = hparams.n_dflash_num_target_layers > 0
                        ? hparams.n_dflash_num_target_layers
                        : 5; // default from config
                    dflash_fc          = create_tensor(tn(LLM_TENSOR_DFLASH_FC,          "weight"), {n_embd * n_target_layers, n_embd}, 0);
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_QWEN3DFLASH:
            {
                llm = std::make_unique<llm_build_qwen3dflash>(*this, params);
            } break;
        case LLM_ARCH_QWEN3MOE:
            {
...
uint32_t llama_model_dflash_block_size(const llama_model * model) {
    return model->hparams.n_dflash_block_size;
}

uint32_t llama_model_dflash_num_target_layers(const llama_model * model) {
    return model->hparams.n_dflash_num_target_layers;
}

int32_t llama_model_dflash_target_layer_ids(const llama_model * model, int32_t * layer_ids, int32_t n_max) {
    const uint32_t n = model->hparams.n_dflash_num_target_layers;
    const int32_t  n_out = (int32_t)std::min((uint32_t)n_max, n);
...
bool llama_model_get_token_embd(const llama_model * model, llama_token token, float * out, int32_t n_embd) {
    if (!model->tok_embd) {
        LLAMA_LOG_ERROR("%s: model has no tok_embd tensor\n", __func__);
        return false;
    }
    const int64_t n_embd_model = model->tok_embd->ne[0];
    const int64_t n_vocab      = model->tok_embd->ne[1];
    if (token < 0 || (int64_t)token >= n_vocab) {
        LLAMA_LOG_ERROR("%s: token %d out of range [0, %lld)\n", __func__, token, (long long)n_vocab);
        return false;
    }
    if (n_embd != (int32_t)n_embd_model) {
        LLAMA_LOG_ERROR("%s: n_embd mismatch: got %d, model has %lld\n", __func__, n_embd, (long long)n_embd_model);
        return false;
    }

    // Use a tiny ggml graph: ggml_get_rows dequantizes the row automatically.
    ggml_backend_t cpu_backend = ggml_backend_cpu_init();
    if (!cpu_backend) { return false; }
    ...
    ggml_set_input(idx);
    struct ggml_tensor * row = ggml_get_rows(ctx, model->tok_embd, idx);
    ggml_set_output(row);
    ...
    // Write token index
    ggml_backend_tensor_set(idx, &token, 0, sizeof(int32_t));

    // Run on CPU — ggml_get_rows on CPU handles GPU-resident tok_embd via host copy
    ggml_backend_graph_compute(cpu_backend, gf);

    // Copy result
    ggml_backend_tensor_get(row, out, 0, (size_t)n_embd * sizeof(float));
...
// GPU-accelerated DFlash fc+norm projection.
// Mirrors llama_lm_head_gpu: pre-built ggml graph, persistent GPU buffers.
// Graph: input [n_in, n_batch] → mul_mat(fc) [n_embd, n_batch] → rms_norm → scale(norm)

struct llama_dflash_proj_gpu {
    ggml_backend_t   backend     = nullptr;
    bool             own_backend = false;
    ggml_context   * ctx         = nullptr;
    ggml_gallocr_t   galloc      = nullptr;
    ggml_cgraph    * gf          = nullptr;
    ggml_tensor    * inp_hidden  = nullptr;  // [n_in,   n_batch]
    ggml_tensor    * out_proj    = nullptr;  // [n_embd, n_batch]
    int32_t n_in = 0, n_embd = 0, n_batch = 0;
    bool ok = false;
};

llama_dflash_proj_gpu * llama_dflash_proj_gpu_create(const llama_model * model, int32_t n_batch,
                                                      ggml_backend_t shared_backend) {
    const ggml_tensor * fc   = model->dflash_fc;
    const ggml_tensor * norm = model->dflash_hidden_norm;
    if (!fc || !fc->buffer || !norm) {
        LLAMA_LOG_WARN("%s: dflash_fc or dflash_hidden_norm missing\n", __func__);
        return nullptr;
    }

    const int64_t n_in   = fc->ne[0];
    ...
    bool own_backend;
    if (shared_backend) {
        // Borrow an existing backend — avoids creating a second CUDA context
        backend     = shared_backend;
        own_backend = false;
    } else {
        ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(fc->buffer);
        ggml_backend_dev_t dev = ggml_backend_buft_get_device(buft);
        if (!dev) {
            LLAMA_LOG_WARN("%s: could not get device from dflash_fc buffer\n", __func__);
            return nullptr;
        }
        backend = ggml_backend_dev_init(dev, nullptr);
        if (!backend) {
            LLAMA_LOG_WARN("%s: failed to init backend for dflash proj\n", __func__);
            return nullptr;
        }
        own_backend = true;
    }

    ggml_init_params p = { 2 * 1024 * 1024, nullptr, /*no_alloc=*/true };
    ggml_context * ctx = ggml_init(p);
    if (!ctx) { ggml_backend_free(backend); return nullptr; }

    ggml_tensor * inp = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_in, n_batch);
    ggml_set_name(inp, "dflash_proj_inp");
    ggml_set_input(inp);

    ggml_tensor * cur = ggml_mul_mat(ctx, const_cast<ggml_tensor *>(fc), inp);
    cur = ggml_rms_norm(ctx, cur, eps);
    cur = ggml_mul(ctx, cur, const_cast<ggml_tensor *>(norm));
    ggml_set_name(cur, "dflash_proj_out");
    ...
    if (!ggml_gallocr_reserve(galloc, gf) || !ggml_gallocr_alloc_graph(galloc, gf)) {
        LLAMA_LOG_WARN("%s: gallocr alloc failed for dflash proj\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx);
    ...
    auto * proj = new llama_dflash_proj_gpu();
    proj->backend     = backend;
    proj->own_backend = own_backend;
    proj->ctx         = ctx;
    proj->galloc      = galloc;
    proj->gf          = gf;
    proj->inp_hidden  = inp;
    proj->out_proj    = cur;
    proj->n_in        = (int32_t)n_in;
    proj->n_embd      = (int32_t)n_embd;
    proj->n_batch     = n_batch;
    proj->ok          = true;

    LLAMA_LOG_INFO("%s: GPU dflash proj ready  n_batch=%d  n_in=%d  n_embd=%d\n",
                   __func__, n_batch, (int)n_in, (int)n_embd);
    return proj;
}

bool llama_dflash_proj_gpu_apply(llama_dflash_proj_gpu * proj,
                                  const float           * hidden_concat,
                                        float           * out,
                                        int32_t           n_tokens) {
    if (!proj || !proj->ok || n_tokens > proj->n_batch) return false;

    const size_t used_in  = (size_t)n_tokens     * proj->n_in   * sizeof(float);
    const size_t total_in = (size_t)proj->n_batch * proj->n_in   * sizeof(float);

    ggml_backend_tensor_set(proj->inp_hidden, hidden_concat, 0, used_in);
    if (used_in < total_in) {
        ggml_backend_tensor_memset(proj->inp_hidden, 0, used_in, total_in - used_in);
    }

    if (ggml_backend_graph_compute(proj->backend, proj->gf) != GGML_STATUS_SUCCESS) {
        return false;
    }

    ggml_backend_tensor_get(proj->out_proj, out, 0,
                            (size_t)n_tokens * proj->n_embd * sizeof(float));
    return true;
}

void llama_dflash_proj_gpu_free(llama_dflash_proj_gpu * proj) {
    if (!proj) return;
    if (proj->galloc)                    ggml_gallocr_free(proj->galloc);
    if (proj->ctx)                       ggml_free(proj->ctx);
    if (proj->backend && proj->own_backend) ggml_backend_free(proj->backend);
    delete proj;
}

// Apply fc (linear) + RMSNorm: out[t] = rms_norm(fc_weight @ hidden_concat[t])
// hidden_concat: [n_tokens, n_in]  (row-major)
// out:           [n_tokens, n_embd] (row-major)
// fc_weight: [n_embd, n_in] stored as ggml tensor (ne[0]=n_in, ne[1]=n_embd)
// norm_weight: [n_embd]
bool llama_model_dflash_project(
        const llama_model * model,
        const float       * hidden_concat,
              float       * out,
              int32_t       n_tokens) {
    if (!model->dflash_fc || !model->dflash_hidden_norm) {
        LLAMA_LOG_ERROR("%s: dflash_fc=%p dflash_hidden_norm=%p\n", __func__,
                        (void*)model->dflash_fc, (void*)model->dflash_hidden_norm);
        return false;
    }
    ...
    const int64_t n_embd = fc->ne[1];   // output dim
    const int64_t n_in   = fc->ne[0];   // input dim = n_target_layers * n_embd

    const float eps = model->hparams.f_norm_rms_eps;

    // Get pointers to weights (assume CPU buffer)
    // Dequantize to F32
    std::vector<float> fc_buf  (n_embd * n_in);
    std::vector<float> norm_buf(n_embd);

    {
        const size_t fc_nbytes = ggml_nbytes(fc);
        std::vector<uint8_t> fc_raw(fc_nbytes);
        ggml_backend_tensor_get(fc, fc_raw.data(), 0, fc_nbytes);
        if (fc->type == GGML_TYPE_F32) {
            memcpy(fc_buf.data(), fc_raw.data(), n_embd * n_in * sizeof(float));
        } else {
            const auto * traits = ggml_get_type_traits(fc->type);
            if (!traits->to_float) {
                LLAMA_LOG_ERROR("%s: unsupported fc type=%d\n", __func__, (int)fc->type);
                return false;
            }
            traits->to_float(fc_raw.data(), fc_buf.data(), n_embd * n_in);
        }
    }

    {
        const size_t norm_nbytes = ggml_nbytes(norm);
        std::vector<uint8_t> norm_raw(norm_nbytes);
        ggml_backend_tensor_get(norm, norm_raw.data(), 0, norm_nbytes);
        if (norm->type == GGML_TYPE_F32) {
            memcpy(norm_buf.data(), norm_raw.data(), n_embd * sizeof(float));
        } else {
            const auto * traits = ggml_get_type_traits(norm->type);
            if (!traits->to_float) {
                LLAMA_LOG_ERROR("%s: unsupported norm type=%d\n", __func__, (int)norm->type);
                return false;
            }
            traits->to_float(norm_raw.data(), norm_buf.data(), n_embd);
        }
    }

    const float * fc_data   = fc_buf.data();
    const float * norm_data = norm_buf.data();

    for (int32_t t = 0; t < n_tokens; t++) {
        const float * x = hidden_concat + (size_t)t * n_in;
        float       * y = out           + (size_t)t * n_embd;

        // y = fc_weight @ x   (fc_weight stored as [n_in, n_embd] in ggml column-major)
        // ggml: ne[0]=n_in, ne[1]=n_embd → row i is fc_data + i*n_in
        for (int64_t i = 0; i < n_embd; i++) {
            float dot = 0.0f;
            const float * row = fc_data + i * n_in;
        ...
        // RMSNorm: y = y / rms(y) * norm_weight
        float sum2 = 0.0f;
        for (int64_t i = 0; i < n_embd; i++) { sum2 += y[i] * y[i]; }
        const float scale = 1.0f / sqrtf(sum2 / (float)n_embd + eps);
        for (int64_t i = 0; i < n_embd; i++) { y[i] *= scale * norm_data[i]; }
    }

    return true;
}

// Apply output_norm + lm_head to embeddings.
// embd:   [n_tokens, n_embd]  row-major F32
// logits: [n_tokens, n_vocab] row-major F32 (caller allocates)
bool llama_model_apply_lm_head(
        const llama_model * model,
        const float       * embd,
              float       * logits,
              int32_t       n_tokens,
              bool          apply_output_norm) {
    const ggml_tensor * output_norm_w = model->output_norm;
    const ggml_tensor * output_w      = model->output;
    ...
    const int64_t n_vocab = output_w->ne[1]; // output: [n_embd, n_vocab] → ne[0]=n_embd, ne[1]=n_vocab

    const float eps = model->hparams.f_norm_rms_eps;

    // Dequantize output_norm weights
    std::vector<float> norm_buf(n_embd, 1.0f); // default: unit weights if no norm
    if (apply_output_norm && output_norm_w) {
        const size_t norm_nbytes = ggml_nbytes(output_norm_w);
        std::vector<uint8_t> norm_raw(norm_nbytes);
        ggml_backend_tensor_get(output_norm_w, norm_raw.data(), 0, norm_nbytes);
        if (output_norm_w->type == GGML_TYPE_F32) {
            memcpy(norm_buf.data(), norm_raw.data(), n_embd * sizeof(float));
        } else {
            const auto * traits = ggml_get_type_traits(output_norm_w->type);
            if (traits->to_float) {
                traits->to_float(norm_raw.data(), norm_buf.data(), n_embd);
            }
        }
    ...
    // Read quantized output (lm_head) weights from GPU: [n_embd, n_vocab]
    // Avoid allocating the full float buffer (n_embd*n_vocab*4 bytes = ~2.4 GB for 248k vocab).
    // Instead: read quantized bytes once, then dequantize in row-chunks.
    const size_t out_nbytes = ggml_nbytes(output_w);
    std::vector<uint8_t> out_raw(out_nbytes);
    ggml_backend_tensor_get(output_w, out_raw.data(), 0, out_nbytes);

    const auto * out_traits = ggml_get_type_traits(output_w->type);
    if (output_w->type != GGML_TYPE_F32 && (!out_traits || !out_traits->to_float)) {
        LLAMA_LOG_ERROR("%s: unsupported output_w type=%d\n", __func__, (int)output_w->type);
        return false;
    }

    // Per-row byte stride in the quantized buffer
    const size_t row_bytes = ggml_row_size(output_w->type, n_embd);

    // RMSNorm all tokens first (if apply_output_norm=true)
    std::vector<float> normed((size_t)n_tokens * n_embd);
    if (apply_output_norm) {
        for (int32_t t = 0; t < n_tokens; t++) {
            const float * x  = embd + (size_t)t * n_embd;
            float       * nx = normed.data() + (size_t)t * n_embd;
            float sum2 = 0.0f;
            for (int64_t i = 0; i < n_embd; i++) { sum2 += x[i] * x[i]; }
            const float scale = 1.0f / sqrtf(sum2 / (float)n_embd + eps);
            for (int64_t i = 0; i < n_embd; i++) { nx[i] = x[i] * scale * norm_buf[i]; }
        }
    } else {
        // No norm: just copy input to normed buffer
        memcpy(normed.data(), embd, (size_t)n_tokens * n_embd * sizeof(float));
    }

    // Chunked matmul: dequantize CHUNK vocab rows at a time
    const int64_t CHUNK = 2048;
    std::vector<float> chunk_buf((size_t)CHUNK * n_embd);

    for (int64_t v0 = 0; v0 < n_vocab; v0 += CHUNK) {
        const int64_t v_end      = std::min(v0 + CHUNK, n_vocab);
        const int64_t chunk_rows = v_end - v0;

        if (output_w->type == GGML_TYPE_F32) {
            memcpy(chunk_buf.data(),
                   out_raw.data() + (size_t)v0 * n_embd * sizeof(float),
                   (size_t)chunk_rows * n_embd * sizeof(float));
        } else {
            out_traits->to_float(out_raw.data() + v0 * row_bytes,
                                 chunk_buf.data(), chunk_rows * n_embd);
        }

        for (int32_t t = 0; t < n_tokens; t++) {
            const float * nx = normed.data() + (size_t)t * n_embd;
            float       *  y = logits + (size_t)t * n_vocab + v0;
            for (int64_t v = 0; v < chunk_rows; v++) {
                float dot = 0.0f;
                const float * row = chunk_buf.data() + (size_t)v * n_embd;
                for (int64_t i = 0; i < n_embd; i++) { dot += nx[i] * row[i]; }
                y[v] = dot;
            }
        }
    }
...
// ── GPU lm_head ────────────────────────────────────────────────────────────
// Builds a ggml compute graph that applies output_norm + output (lm_head)
// on the GPU, avoiding CPU matmul and GPU→CPU weight copies.
...
llama_lm_head_gpu * llama_lm_head_gpu_create(const llama_model * model, int32_t n_batch, bool apply_output_norm) {
    const ggml_tensor * output_w      = model->output;
    const ggml_tensor * output_norm_w = model->output_norm;

    if (!output_w || !output_w->buffer) {
        LLAMA_LOG_WARN("%s: output weight not on GPU buffer — GPU lm_head unavailable\n", __func__);
        return nullptr;
    }

    const int64_t n_embd  = (int64_t)model->hparams.n_embd;
    const int64_t n_vocab = output_w->ne[1];
    const float   eps     = model->hparams.f_norm_rms_eps;

    // Derive the compute backend from output_w's buffer type (no CUDA header needed)
    ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(output_w->buffer);
    ggml_backend_dev_t dev = ggml_backend_buft_get_device(buft);
    if (!dev) {
        LLAMA_LOG_WARN("%s: could not get device from output weight buffer\n", __func__);
        return nullptr;
    }
    ggml_backend_t backend = ggml_backend_dev_init(dev, nullptr);
    if (!backend) {
        LLAMA_LOG_WARN("%s: failed to init backend\n", __func__);
        return nullptr;
    }

    // Build graph in a ggml_context with no_alloc=true (gallocr allocates later)
    ggml_init_params p = { 2 * 1024 * 1024, nullptr, /*no_alloc=*/true };
    ggml_context * ctx = ggml_init(p);
    if (!ctx) {
        ggml_backend_free(backend);
    ...
    // Input: noise embeddings [n_embd, n_batch]
    ggml_tensor * inp_embd = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, n_batch);
    ggml_set_name(inp_embd, "lmh_inp");
    ggml_set_input(inp_embd);

    ggml_tensor * cur = inp_embd;

    // RMSNorm + scale (only if apply_output_norm=true)
    if (apply_output_norm) {
        cur = ggml_rms_norm(ctx, cur, eps);
        if (output_norm_w) {
            cur = ggml_mul(ctx, cur, const_cast<ggml_tensor *>(output_norm_w));
        }
    }

    // matmul: output_w [n_embd, n_vocab] × cur [n_embd, n_batch] → [n_vocab, n_batch]
    ...
    ggml_gallocr_t galloc = ggml_gallocr_new(buft);
    if (!ggml_gallocr_reserve(galloc, gf)) {
        LLAMA_LOG_WARN("%s: gallocr reserve failed\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx);
        ggml_backend_free(backend);
        return nullptr;
    ...
    if (!ggml_gallocr_alloc_graph(galloc, gf)) {
        LLAMA_LOG_WARN("%s: gallocr alloc failed\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx);
        ggml_backend_free(backend);
        return nullptr;
    ...
    auto * lmh = new llama_lm_head_gpu();
    lmh->backend     = backend;
    lmh->own_backend = true;
    lmh->ctx         = ctx;
    lmh->galloc      = galloc;
    lmh->gf          = gf;
    ...
    LLAMA_LOG_INFO("%s: GPU lm_head ready  n_batch=%d  n_embd=%d  n_vocab=%d\n",
                   __func__, n_batch, (int)n_embd, (int)n_vocab);
    return lmh;
}

bool llama_lm_head_gpu_apply(llama_lm_head_gpu * lmh,
                              const float       * embd,
                                    float       * logits,
                                    int32_t       n_tokens) {
    ...
    // Run the graph on the GPU backend
...
void llama_lm_head_gpu_free(llama_lm_head_gpu * lmh) {
    if (!lmh) return;
    if (lmh->galloc)                ggml_gallocr_free(lmh->galloc);
    if (lmh->ctx)                   ggml_free(lmh->ctx);
    if (lmh->backend && lmh->own_backend) ggml_backend_free(lmh->backend);
    delete lmh;
}

ggml_backend_t llama_lm_head_gpu_get_backend(llama_lm_head_gpu * lmh) {
    return lmh ? lmh->backend : nullptr;
}
