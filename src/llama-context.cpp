...
float * llama_context::get_layer_hidden(int32_t layer_idx) {
    auto it = hidden_states_out.find(layer_idx);
    if (it == hidden_states_out.end() || it->second.empty()) {
        return nullptr;
    }
    return it->second.data();
}
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        // Prepare per-layer hidden state capture slots (DFlash support).
        // Must be set before model.build_graph so graph builders can populate them.
        if (!hidden_capture_layers.empty()) {
            const int32_t n_layer = (int32_t)model.hparams.n_layer;
            res->t_layer_hidden.assign(n_layer, nullptr);
        } else {
            res->t_layer_hidden.clear();
        }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        if (!hidden_capture_layers.empty()) {
            ...
            for (int32_t il : hidden_capture_layers) {
                auto * t_hidden = res->get_layer_hidden(il);
                if (t_hidden) {
