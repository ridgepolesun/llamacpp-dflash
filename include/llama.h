...
extern "C" {
    ...
    // DFlash draft model queries
    // Returns 0 if the model is not a DFlash draft model.
    LLAMA_API uint32_t llama_model_dflash_block_size       (const struct llama_model * model);
    LLAMA_API uint32_t llama_model_dflash_num_target_layers(const struct llama_model * model);
    // Fills layer_ids[0..n_out-1] with the target layer indices. Returns count written.
    LLAMA_API int32_t  llama_model_dflash_target_layer_ids (const struct llama_model * model, int32_t * layer_ids, int32_t n_max);

    // Copy the embedding vector for a single token from the model's tok_embd table.
    // out must point to a buffer of at least n_embd floats.
    // Returns false if the token is out of range or the model has no embedding table.
    LLAMA_API bool llama_model_get_token_embd(
        const struct llama_model * model,
        llama_token                token,
        float                    * out,
        int32_t                    n_embd);

    // Apply the DFlash fc+hidden_norm projection to concatenated target hidden states.
    // Input:  hidden_concat [n_embd * n_target_layers, n_tokens] row-major float
    // Output: out          [n_embd, n_tokens] row-major float (caller allocates)
    // The draft model must be QWEN3DFLASH arch. Returns false on error.
    LLAMA_API bool llama_model_dflash_project(
        const struct llama_model * draft_model,
        const float              * hidden_concat, // [n_tokens, n_embd * n_target_layers]
              float              * out,           // [n_tokens, n_embd]
              int32_t              n_tokens);

    // Apply lm_head (output projection) to embeddings.
    // If apply_output_norm=true (default): computes logits[t] = output_weight @ rms_norm(embd[t]).
    // If apply_output_norm=false: computes logits[t] = output_weight @ embd[t] (no norm).
    //   Use false when embeddings are already normalized (e.g. DFlash draft output).
    // embd:   [n_tokens, n_embd]  row-major float
    // logits: [n_tokens, n_vocab] row-major float (caller allocates)
    // Returns false if model has no output projection (e.g. embedding-only model).
    LLAMA_API bool llama_model_apply_lm_head(
        const struct llama_model * model,
    ...
    // GPU-accelerated lm_head handle. Builds a ggml compute graph using the model's
    // output weights (already on GPU) to apply lm_head entirely on GPU.
    // n_batch is the fixed batch size (must match calls to llama_lm_head_gpu_apply).
    // apply_output_norm: if true, applies output_norm before the linear projection.
    //   Set false when embeddings are already normalized (e.g. DFlash draft output).
    struct llama_lm_head_gpu;
    LLAMA_API struct llama_lm_head_gpu * llama_lm_head_gpu_create(
        const struct llama_model * model, int32_t n_batch, bool apply_output_norm);
    LLAMA_API bool llama_lm_head_gpu_apply(
    ...
    // Computes: out[t] = rms_norm(fc @ hidden_concat[t]) * norm_weight
    // hidden_concat: host float [n_tokens, n_in]
    // out:           host float [n_tokens, n_embd]
    // shared_backend: if non-NULL, borrow this backend (avoids creating a second CUDA context).
    //   Pass llama_lm_head_gpu_get_backend(lmh) to share with the lm_head GPU.
    struct llama_dflash_proj_gpu;
    LLAMA_API struct llama_dflash_proj_gpu * llama_dflash_proj_gpu_create(
        const struct llama_model * model, int32_t n_batch,
        ggml_backend_t shared_backend);
    LLAMA_API bool llama_dflash_proj_gpu_apply(
        struct llama_dflash_proj_gpu * proj,
        const float                  * hidden_concat,
              float                  * out,
              int32_t                  n_tokens);
    LLAMA_API void llama_dflash_proj_gpu_free(struct llama_dflash_proj_gpu * proj);
...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    // DFlash: configure which transformer layer indices to capture hidden states for.
    // Must be called before llama_decode(). layer_ids is a list of layer indices (0-based).
    // After decode, use llama_get_layer_hidden() to retrieve the captured states.
    LLAMA_API void llama_set_hidden_capture_layers(struct llama_context * ctx, const int32_t * layer_ids, int32_t n_layers);

    // DFlash: get pointer to hidden states captured at layer_idx after last llama_decode().
